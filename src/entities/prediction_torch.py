...
class SinglePredictionResultTorch(BaseModel):
    """
...
class PredictResponseTorch(BaseModel):
    """
    A model representing the response of a prediction request.

    Attributes:
        results (List[SinglePredictionResultTorch]): A list of results for each prediction made.
        message (str, optional): A message associated with the prediction response.
    """
    results: List[SinglePredictionResultTorch]
    message: str = None
    # other: Any = None